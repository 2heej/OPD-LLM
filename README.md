...
이 프로젝트는 위 네 가지 문제를 LLM 기반 구조화 요약으로 완화할 수 있는지를, **실제 서비스 구축이 아니라 프롬프트 설계·평가 관점**에서 검증하는 NLP/LLM 포트폴리오 프로젝트다. 합성 문진 케이스 30개(개발 20/시험 10)에 대해 프롬프트 v0~v3의 정보 추출 성능을 정량 지표와 오류 분석으로 비교한다.
...
| 실험 관리 (선택) | LangSmith | 호출 트레이싱, 데이터셋·실험 기록, 버전 비교 대시보드 |
...
개발 세트 20케이스 × v0~v3 비교 결과(2차 실행). 상세 수치는 `results/metrics_summary.csv`, 해석은 노트북 10번 섹션 참고.
...
- 필드·허용값 명시(v1)가 가장 큰 개선을 만들었고, 불확실성 규칙(v2)은 오히려 v1보다 하락 — '명시 없으면 유보' 규칙이 지저분한 문장에서 명시된 정보까지 유보하게 만드는 부작용이 확인됐다. 도메인 규칙(v3)은 Micro F1과 누락률에서 최고 성적으로, 누락 최소화가 최우선인 문진 목적에 맞아 최종 선택했다.
- 최종 버전(v3)의 시험 세트(10케이스) 성적: Exact Match 40%, Field Accuracy 83.3%, Micro F1 0.912, 방문 형태 정확도 100%. 남은 오류는 증상 변화 경계(나빠짐 vs 새 증상), 서류 발급 vs 결과 확인 혼동, 다중 요청 일부 누락에 몰려 있으며 `results/error_analysis.csv`에 정리했다.
- 1차 파일럿(10케이스 쉬운 세트)에서는 v3가 Exact Match 85.7%, Micro F1 1.000이었다. 지저분한 문장·희소 라벨·다중 요청을 넣자 점수가 내려간 것으로, 쉬운 세트의 높은 점수가 일반화를 보장하지 않음을 보여준다.
- LangSmith 연동 시(선택) 모든 호출이 트레이싱되고, 개발 세트가 데이터셋·실험으로 기록되어 v0~v3를 화면에서 비교할 수 있다(노트북 11번 섹션).
...
- 합성 케이스 30개(개발 20 / 시험 10)로 구성된 소규모 평가이며, 실제 외래 표현 분포를 대표하지 않는다.
- 증상 변화의 '나빠짐'과 '새 증상 있음' 경계처럼 사람 평가자 간에도 불일치가 예상되는 라벨이 있다('양상 변화' 중간 카테고리 신설 검토 중).
