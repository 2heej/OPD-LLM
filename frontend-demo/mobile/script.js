...
//
// 서버·LLM을 호출하지 않는 정적 프로토타입이다. 접수를 마치고 대기실에서
// 본인 휴대폰으로 작성하는 상황을 가정한다(이미 병원에 도착한 뒤). 화면
// 전환·검증 로직만 담당하며, 마지막 화면의 개발자용 토글에서만
// NLP_LLM_Prompt_Evaluation.ipynb가 받는 것과 같은 스키마의 JSON을 확인할 수 있다.
...
function screens() {
  return {
    ...
    review() {
      ...
      pre.textContent = JSON.stringify(answers, null, 2);
